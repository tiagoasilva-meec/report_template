...
\textit{Versat} is a novel Coarse-Grained Reconfigurable Architecture (CGRA) designed for embedded systems. It addresses the challenge of accelerating compute-intensive inner loops in code while efficiently managing the transitions between code suitable for the CGRA and code that needs to run on the host processor~\cite{paperversat}.\\
A list of the key features is presented below:

\begin{itemize}
    \item \textbf{Nested Loop Support}: \textit{Versat} incorporates a programmable controller that handles reconfiguration, data transfer, and simple control code between the accelerated program loops, reducing the host processor's frequent interventions.
    \item \textbf{Self-Reconfiguration}: \textit{Versat} offers self-reconfiguration, allowing it to generate its configurations, which are more efficient to store than pre-defined configurations. This self-reconfiguration reduces the overhead of frequent configuration changes.
