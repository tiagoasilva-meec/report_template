...
\textit{Versat} is compatible with IOb-Soc. Figure \ref{fig:iobversat} shows the high-level block diagram of IOb-SoC and \textit{Versat}.

\vspace{0.1cm}

\begin{figure}[H]
\centerline{\includegraphics[width=0.85\linewidth]{iob-soc-versat.pdf}}
\caption{IOb-SoC and \textit{Versat} and high-level block diagram.}
\label{fig:iobversat}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|p{0.8\linewidth}|}
        ...
        The AGUs can be configured to output or to store data (only one type per port, the same port cannot be configured to output and store data at the same time in one run). \\
        \hline
